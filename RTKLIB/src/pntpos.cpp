...
static int rescode(int iter, const obsd_t *obs, int n, const double *rs,
                   ...
                   const nav_t *nav, const double *x, const prcopt_t *opt,
                   double *v, double *H, double *var, double *azel, int *vsat,
                   double *resp, int *ns, double *my_prng)
{
    gtime_t time;
    double r,freq,dion,dtrp,vmeas,vion,vtrp,rr[3],pos[3],dtr,e[3],P;
    int i,j,nv=0,sat,sys,mask[NX-3]={0};
    ...
    for (i=*ns=0;i<n&&i<MAXOBS;i++) {
        ...
        vsat[i]=1; resp[i]=v[nv]; (*ns)++; my_prng[i] = tmp;
...
static int estpos(const obsd_t *obs, int n, const double *rs, const double *dts,
                  ...
                  double *resp, char *msg, MyOptimization & myOptimization)
{
    ...
    for (i=0;i<MAXITR;i++) {
        ...
        if ((info=lsq(H,v,NX,nv,dx,Q))) {
            sprintf(msg,"my lsq error info=%d",info);
            break;
        }
        for (j=0;j<NX;j++) {
            x[j]+=dx[j];
